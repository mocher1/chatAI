/*
  # Analytics and Job Market Database Schema

  1. New Tables
    - `chat_interactions` - Stores user chat interactions with analytics data
    - `user_sessions` - Tracks user sessions and activity
    - `prompt_variants` - A/B testing for different prompt versions
    - `performance_metrics` - System performance tracking
    - `popular_questions` - Aggregated popular questions and categories

  2. Security
    - Enable RLS on all tables
    - Add policies for public read/write access where appropriate
    - Add policies for authenticated users for prompt variants

  3. Functions and Triggers
    - Auto-update timestamp triggers
    - Popular questions aggregation trigger
...
-- Enable RLS for chat_interactions
ALTER TABLE chat_interactions ENABLE ROW LEVEL SECURITY;

-- Create policies for chat_interactions (drop if exists first)
DO $$
BEGIN
    DROP POLICY IF EXISTS "Public can insert chat interactions" ON chat_interactions;
...
DO $$
BEGIN
    DROP POLICY IF EXISTS "Public can read chat interactions" ON chat_interactions;
...
DO $$
BEGIN
    DROP POLICY IF EXISTS "Public can update chat interactions" ON chat_interactions;
...
DO $$
BEGIN
    DROP POLICY IF EXISTS "Public can insert user sessions" ON user_sessions;
...
-- Enable RLS for prompt_variants
ALTER TABLE prompt_variants ENABLE ROW LEVEL SECURITY;

-- Create policies for prompt_variants
DO $$
BEGIN
    DROP POLICY IF EXISTS "App can read active prompt variants" ON prompt_variants;
...
DO $$
BEGIN
    DROP POLICY IF EXISTS "Public read access to performance metrics" ON performance_metrics;
...
-- Enable RLS for popular_questions
ALTER TABLE popular_questions ENABLE ROW LEVEL SECURITY;

-- Create policies for popular_questions
DO $$
BEGIN
    DROP POLICY IF EXISTS "Public read access to popular questions" ON popular_questions;
...
DO $$
BEGIN
    DROP TRIGGER IF EXISTS set_chat_interactions_updated_at ON chat_interactions;
...
DO $$
BEGIN
    DROP TRIGGER IF EXISTS set_prompt_variants_updated_at ON prompt_variants;
...
DO $$
BEGIN
    DROP TRIGGER IF EXISTS set_popular_questions_updated_at ON popular_questions;
...
DO $$
BEGIN
    DROP TRIGGER IF EXISTS trigger_update_popular_questions ON chat_interactions;
